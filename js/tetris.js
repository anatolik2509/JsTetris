...
function startFigureFalling(state) {
    ...
    cycle = setInterval(() => {
        ...
        if (allowedState(newState) && maxRetries > 0) {
...
function renderState(oldState, newState) {
    for (let i = 0; i < oldState.figure.length; i++) {
        ...
        addColorOnSquare(pointPos, 'black')
    }
...
function transformLeft(figure) {
    let newFigure = []
    for (let i = 0; i < figure.length; i++) {
        newFigure.push([figure[i][1], figure[i][0] * -1])
    }
    return newFigure
}
...
function canMoveFigure(figure, figurePos, dx, dy) {
    newPos = [figurePos[0] + dx, figurePos[1] + dy]
    return allowedState(figure, newPos)
}
